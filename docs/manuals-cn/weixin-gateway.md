...
本指南介绍如何将微信 iLink 机器人账号连接到 Pi WebUI，以便发送微信消息，并接收 Pi 智能体服务器的回复。

## 网关的工作方式

微信网关使用腾讯 iLink 机器人 API。与飞书网关不同，它不提供公开的 Webhook 回调。Pi WebUI 会启动一个后台轮询器：
...
- 必须设置 `PI_WEBUI_WECHAT_GATEWAY_ENABLED=true`。仅完成配对只会保存凭据；只有启用网关后，轮询器才会启动。
...
确认后，Pi WebUI 会将 iLink 账号凭据保存到本地数据库中。通常无需手动设置 `PI_WEBUI_WECHAT_ACCOUNT_ID` 或 `PI_WEBUI_WECHAT_TOKEN`。
...
当微信/iLink 在 `voice_item.text` 中包含语音识别文本时，也支持语音消息。在这种情况下，Pi WebUI 会像处理输入的文本消息一样，将识别出的文本发送给智能体。
...
- `/clear-config` 删除该聊天的覆盖设置，并恢复 `PI_WEBUI_WECHAT_*` 环境变量中的默认值。
- `/new` 和 `/reset` 会释放该微信聊天当前的内存中 Pi 会话，并将下一条消息绑定到新的 Pi 会话。

## 私聊访问策略

`PI_WEBUI_WECHAT_DM_POLICY` 控制哪些用户可以与机器人聊天：

| 值 | 行为 |
|---|---|
| `pairing` | 默认值。接受传入的私聊消息，适合在设置和查找发送者 ID 时使用。 |
| `allowlist` | 仅接受 `PI_WEBUI_WECHAT_ALLOWED_USERS` 中列出的用户。 |
| `open` | 只有同时设置 `PI_WEBUI_WECHAT_ALLOW_ALL_USERS=true` 或 `GATEWAY_ALLOW_ALL_USERS=true` 时，才接受所有用户。 |
| `disabled` | 忽略所有私聊消息。 |
...
- `PI_WEBUI_WECHAT_GATEWAY_ENABLED` 未设置为 `true`。
- 更改 `.env` 后未重启服务器。
...
如果日志中的 `voice_item` 包含 `text` 字段，但值为空，说明微信/iLink 没有为该语音消息提供转写。Pi WebUI 目前不会自行下载音频和执行 ASR。
...
- 不要提交 `.env`，其中可能包含网关凭据。
- 保存的微信配对凭据存储在本地 Pi WebUI 数据库中。
- 如果不受信任的用户可能访问机器人账号，请在设置完成后使用 `PI_WEBUI_WECHAT_DM_POLICY=allowlist` 限制私聊访问。
