...
The Worktree feature lets Pi WebUI prepare an isolated Git worktree when you create a Pi session instead of working directly in the current project directory. A single repository can then support multiple branches, directories, and Pi sessions in parallel without overwriting the same workspace files.
...
3. Under **Work location**, choose:
   ...
   - **Managed Git worktree**: Use a Git worktree managed by Pi WebUI.
...
Pi WebUI performs an operation equivalent to:
...
For **Existing branch**, select a local branch. Pi WebUI:
...
Pi WebUI scans ordinary files at the project root that Git ignores and lets you copy one of them into the worktree root.
...
When a managed worktree session is complete, use **Finish worktree**. Pi WebUI first displays a cleanup preview showing:
...
Git worktree is a built-in Git feature that checks out different branches from one repository into multiple working directories. Pi WebUI invokes Git commands rather than copying the entire repository.
...
- `worktreeManaged`: Identifies a Pi WebUI-managed worktree.
...
For managed worktree sessions, Pi WebUI associates project memory with `baseRepoPath`, not the temporary `worktreePath`. The base repository and its managed worktrees therefore share project memory.
...
| --- | --- | --- |
| Working directory | Current project directory | Worktree created or reused by Pi WebUI |
...
- Worktrees are available only for Git repositories. If branches cannot be loaded, Pi WebUI hides the managed worktree option.
- The branch list contains local branches only. Fetch and check out a remote branch locally before selecting it.
- Git normally prevents one branch from being checked out in multiple worktrees; Pi WebUI reuses an existing worktree when possible.
